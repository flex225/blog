            ...
            <div class="col-md-10 col-md-offset-1">
                ...
                <div class="row">
                    <div class="form-group col-md-3" style="float: left;margin-right:20px">
                        <select name='gender' class="form-control input-md need colorChange" style="color:grey;">
                    ...
                    <div class="form-group col-xs-4" style="min-width:200px;">
                        <select name='category' id="category" class="form-control input-md need colorChange" style="color:grey;">
                            <option value="-1">Բաժին*</option>
                            @foreach($post_category as $category)
                                <option value="{{$category->id}}">{{$category->category_name}}</option>
                            @endforeach
                        </select>
                        <select name='subcategory1' id="subcategory1" class="form-control input-md need colorChange" style="color:grey;" disabled>
                            <option value="-1">Ենթաբաժին*</option>
                        </select>
                        <select name='subcategory2' id="subcategory2" class="form-control input-md need colorChange" style="color:grey;" disabled>
                            <option value="-1">Ենթաբաժին</option>
                        </select>

                    </div>
                    <div class="form-group col-md-3" style="float: left;margin-right:20px">
                        <select name='age' class="form-control input-md help colorChange" style="color:grey;">
                            <option value="-1">Տարիք</option>
                            @foreach($ages as $age)
                                <option value="{{$age->id}}">{{$age->age}}</option>
                            @endforeach
                        </select>
                        <select name='status' class="form-control input-md help colorChange" style="color:grey;">
            ...
            $('#category').change(function () {
               if(this.value != -1) {
                   ...
                   $.get( "{{ route('getSubcategories') }}",{id:value}, function( data ) {
                       ...
                       $( "#subcategory2" ).html( first + data );
                       $( "#subcategory1" ).html(second + data ).prop("disabled", false);
                   });
               } else {
                   $( "#subcategory1" ).prop("disabled", true);
               }
            });
